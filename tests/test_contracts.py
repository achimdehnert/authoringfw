...
def test_should_verify_llm_result_field_types():
    ...
    result = LLMResult(
        success=True, content="ok", model="gpt-4o", input_tokens=10, output_tokens=5, latency_ms=200
    )
...
def test_should_raise_configuration_error_when_aifw_not_installed():
    ...
    with patch.object(
        orch, "_get_action_config", side_effect=ConfigurationError("aifw not installed")
    ):
